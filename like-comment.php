...
if(!empty($_POST['comment_id']) && !empty($_POST['user_id'])){
  ...
  if($mysqli){
    ...
    if($prequery->num_rows > 0){
      ...
      $presql2 = $mysqli->prepare("SELECT * FROM post_comment_like WHERE comment_id = ? AND user_like = ? AND Status <> 'Delete' ");
      $presql2->bind_param("ii",$comment_id,$myid);
      $presql2->execute();
      $prequery2 = $presql2->get_result();

      if($prequery2->num_rows == 0){
        $sql = $mysqli->prepare("INSERT INTO post_comment_like(comment_id,user_like,CreateDate,CreateIP,Status)  VALUES(?,?,?,?,?)");
        ...
        $sql = $mysqli->prepare("UPDATE post_comment_like SET Status=? WHERE comment_id=? AND user_like=?");
      ...
      if($showData){
        $sqlF1 = $mysqli->prepare("SELECT COUNT(user_like) as comment_like FROM post_comment_like WHERE comment_id = ? AND user_like <> 0 AND Status <> 'Delete' ");
