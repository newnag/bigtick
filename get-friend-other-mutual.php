    ...
    if($mysqli){
      ...
      if($query->num_rows>0){
        ...
        while($Row = $query->fetch_assoc()){
          $sql_1 = $mysqli->prepare("SELECT * FROM friend WHERE user_id = ? AND friend_id = ? AND Status <> 'Delete' ");
          $sql_1->bind_param("ii",$Row["friend_id"],$myid);
          $sql_1->execute();
          $query_1  =   $sql_1->get_result();
          $Row_1 = $query_1->fetch_assoc();

          $user_data = get_user($Row_1['user_id']);
