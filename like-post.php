...
if(!empty($_POST['post_id']) && !empty($_POST['user_id'])){
  ...
  if($mysqli){
    ...
    if($prequery->num_rows > 0){
      ...
      $presql2 = $mysqli->prepare("SELECT * FROM post_comment WHERE post_id = ? AND user_like = ? AND Status <> 'Delete' ");

      $presql2->bind_param("ii",$post_id,$myid);
      ...
      if($prequery2->num_rows == 0){
        ...
        $sql = $mysqli->prepare("INSERT INTO post_comment(post_id,user_like,CreateDate,CreateIP,Status)  VALUES(?,?,?,?,?)");

        $sql->bind_param("iisss",$post_id,$myid,$createdate,$create_ip,$status);
        ...
        $sql = $mysqli->prepare("UPDATE post_comment SET Status=? WHERE post_id=? AND user_like=?");
