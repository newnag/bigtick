...
if(!empty($_POST['post_id'])){
    ...
    if($mysqli){
      ...
      $sql = $mysqli->prepare("SELECT * FROM post WHERE id = ? AND Status <> 'Delete' ");
      ...
      if($query->num_rows>0){
        ...
        while($Row = $query->fetch_assoc()){
          ...
          if($Row['group_vote'] != ''){
            ...
            $id_groupVote = $Row['group_vote'];

    

            $sql_groupVote = $mysqli->prepare("SELECT * FROM type_vote WHERE id IN (".$id_groupVote.") ");

            $sql_groupVote->execute();

            $query_groupVote = $sql_groupVote->get_result();

            if($query_groupVote->num_rows > 0){

              while($Row_groupVote = $query_groupVote->fetch_assoc()){

                $Arr_groupVote[] = $Row_groupVote;

              }

              $arrRow["group_vote"] = $Arr_groupVote;

            }

            else{

              $arrRow["group_vote"] = null;
