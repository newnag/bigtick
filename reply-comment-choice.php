...
if(!empty($_POST['choice_comment_id']) && !empty($_POST['user_id'])){
  ...
  if($mysqli){
    ...
    $presql = $mysqli->prepare("SELECT * FROM post_choice_comment_user WHERE id = ? AND Status <> 'Delete' ");
    ...
    if($prequery->num_rows > 0){
      ...
      $sql = $mysqli->prepare("INSERT INTO post_choice_comment_reply(choice_comment_id,user_id,text,picture,CreateDate,CreateIP,Status)  VALUES(?,?,?,?,?,?,?)");

      $sql->bind_param("iisssss",$choice_comment_id,$myid,$text,$picture,$createdate,$create_ip,$status);
