...
if($mysqli){
  ...
  if($myid != ""){
    $sql = $mysqli->prepare("SELECT g.* FROM group_user as g JOIN group_member as member ON g.id = member.group_id WHERE member.user_id = ? AND g.Status <> 'Delete' AND member.Status <> 'Delete' AND member.request_status IN ('Approve','Accept')");
    $sql->bind_param('i',$myid);
    $sql->execute();
    $query  =   $sql->get_result();
  }
  else{
    $sql = $mysqli->prepare("SELECT * FROM group_user WHERE Status <> 'Delete' ");
    $sql->execute();
    $query  =   $sql->get_result();
  }
  ...
  if($query->num_rows>0){
    ...
    while($Row = $query->fetch_assoc()){
      ...
      $sql3->bind_param("ii",$Row['id'],$my_id);
      $sql3->execute();
