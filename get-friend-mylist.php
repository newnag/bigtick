...
if(!empty($_POST['user_id'])){
    ...
    if($mysqli){
      ...
      $sql = $mysqli->prepare("SELECT * FROM friend WHERE user_id = ? AND request_action = 'Accept' AND friend_id NOT IN ((SELECT user_id FROM group_member WHERE group_member.group_id = ? AND group_member.request_status IN ('Approve','Accept','Inviting','Requesting') AND group_member.user_id <> ? AND group_member.Status <> 'Delete')) AND Status <> 'Delete'");
      $sql->bind_param("iii",$myid,$group_id,$myid);
      $sql->execute();
      ...
      if($query->num_rows>0){
        ...
        while($Row = $query->fetch_assoc()){
          ...
          $preSqlBlock->bind_param("ii",$myid,$user_data["id"]);
          $preSqlBlock->execute();
          ...
          $preSql1->bind_param("ii",$myid,$user_data["id"]);
          $preSql1->execute();
          ...
          $sql_friend_request_group->bind_param("ii",$user_data["id"],$group_id);
          ...
          if($Prequery_friend_request_group->num_rows > 0){
            if($Row_friend_request_group['request_status'] == "Inviting" || $Row_friend_request_group['request_status'] == "Requesting"){
              $arrData["request_group_join"] = true;
              $arrData["request_group_join_id"] = $group_id;

              $arrData["in_group"] = false;
              $arrData["in_group_id"] = $group_id;
            }
            elseif($Row_friend_request_group['request_status'] == "Approve" || $Row_friend_request_group['request_status'] == "Accept"){
              $arrData["request_group_join"] = false;
              $arrData["request_group_join_id"] = $group_id;

              $arrData["in_group"] = true;
              $arrData["in_group_id"] = $group_id;
            }
          }
          else{
            $arrData["request_group_join"] = false;
            $arrData["request_group_join_id"] = null;
            $arrData["in_group"] = false;
            $arrData["in_group_id"] = null;
          }
          ////////////////////////////////////////////////////////////////////////////////////////////
          $FaarRow[] = $arrData;
