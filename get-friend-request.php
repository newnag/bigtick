...
if(!empty($_POST['user_id'])){
    ...
    if($mysqli){
      ...
      $sql = $mysqli->prepare("SELECT * FROM friend WHERE friend_id = ? AND request_action <> 'Accept' AND Status <> 'Delete' ");
      ...
      if($query->num_rows>0){
        while($Row = $query->fetch_assoc()){
          $preCheck = $mysqli->prepare("SELECT * FROM friend WHERE user_id = ? AND friend_id = ? AND request_action = 'Block' AND Status <> 'Delete' ");
          $preCheck->bind_param("ii",$myid,$Row['user_id']);
          $preCheck->execute();
          $PreCheck_query = $preCheck->get_result();

          if($PreCheck_query->num_rows == 0){
